...
local mysqlhost1 = "78.143.2.10"
local mysqluser1 = "ni46611_2_DB"
local mysqlpassword1 = "iOQjCbUn"
local mysqldatabase1 = "ni46611_2_DB"

-- SECOND CONNECTION, OPTIONAL IF CONNECTION 1 DON'T WORK

local mysqlhost2 = "128.65.217.139"
local mysqluser2 = "ni46611_1_DB"
local mysqlpassword2 = "A0IrVpbv"
local mysqldatabase2 = "ni46611_1_DB"
...
preis["heilung"] = 50 -- Preis fuer die Heilung
...
addEventHandler("doHandyKickAntiSpam", getRootElement(), function()
	...
	outputServerLog(name.." wurde wegen Spam vom Server gekickt")
...
FUNC["loadupdates"] = function()
	...
	if(result) and (numrows > 0) then
		...
		updatetext = text
		outputDebugString("Updatetext geladen!")
		--outputDebugString("Updatetext geladen!: "..text)
		dbFree(query)
	else
		outputDebugString("Kein Updatetable vorhanden. Erstelle neue...")
		query = dbQuery(ghandler, "INSERT INTO galaxy_updates (DATA) values ('Frische Updates kommen immer von Leyynen!');")
		result = dbPoll(query, dbpTime)
		if(result) then
			outputDebugString("Updates in den Table erfolgreich insertet!")
			dbFree(query)
			FUNC["loadupdates"]()
		end
	end
...
FUNC["updateupdates"] = function(thePlayer, text)
	...
	if(result) then
		updatetext = text
		outputServerLog("Die Updates wurden von "..getPlayerName(thePlayer).." bearbeitet.")
...
addEventHandler("onResourceStart", getResourceRootElement(), function()
	...
	if not(ghandler) then	
		...
		if not(ghandler) then
			outputServerLog("[Handy]MySQL handler 2 not accepted! Shutting down...")
			cancelEvent()
		else
			FUNC["loadupdates"]()
			outputServerLog("[Handy]MySQL handler 2 accepted!")
		end
	else
		FUNC["loadupdates"]()
		outputServerLog("[Handy]MySQL handler 1 accepted!")
	end
end)
...
if getElementInterior ( source ) == 0 and getElementDimension ( source ) == 0 then
	...
	setTimer(function()
		...
		setTimer(function()
			...
			setElementData(pickup, "menge", menge)
			addEventHandler("onPickupHit", pickup, function(hitElement)
				if(getElementType(hitElement) == "player") and (getElementData(source, "for") == getPlayerName(hitElement)) then
					...
					outputChatBox("Geldlieferung erhalten!", hitElement, 0, 150, 0)
				end
			end)
		end, 10000, 1)
	end, delivertime/2, 1)
		...
		setTimer(function()
			...
			addEventHandler("onPickupHit", pickup, function(hitElement)
				if(getElementType(hitElement) == "player") then
					...
					local weap = "9mm"
					if(accept2[weap] == true) then
						if(accept[weap] == true) then
							giveWeapon(hitElement, 22, 17*accept2[weap])
						else
							if (getPedWeapon (hitElement, 2) == 22) then
								giveWeapon(hitElement, 22, 17*accept2[weap])							
							elseif (getPedWeapon (hitElement, 2) == 23) then
								giveWeapon(hitElement, 23, 17*accept2[weap])
							end
						end
					end
					weap = "deagle"
					if(accept2[weap] == true) then
						if(accept[weap] == true) then
							giveWeapon(hitElement, 24, 7*accept2[weap])
						else
							if (getPedWeapon (hitElement, 2) == 24) then
								giveWeapon(hitElement, 24, 7*accept2[weap])
							end
						end
					end
					...
					if(accept2[weap] == true) then
						if(accept[weap] == true) then
							giveWeapon(hitElement, 25, 5+accept2[weap])
						else
							if (getPedWeapon (hitElement, 3) == 25) then
								giveWeapon(hitElement, 25, 5+accept2[weap])
							elseif (getPedWeapon (hitElement, 3) == 26) then
								giveWeapon(hitElement, 26, 5+accept2[weap])
							elseif (getPedWeapon (hitElement, 3) == 27) then
								giveWeapon(hitElement, 27, 5+accept2[weap])
							end
						end
					end
					weap = "mp5"
					if(accept2[weap] == true) then
						if(accept[weap] == true) then
							giveWeapon(hitElement, 29, 30*accept2[weap])
						else
							if (getPedWeapon (hitElement, 4) == 29) then
								giveWeapon(hitElement, 29, 30*accept2[weap])
							elseif (getPedWeapon (hitElement, 4) == 28) then
								giveWeapon(hitElement, 28, 30*accept2[weap])
							elseif (getPedWeapon (hitElement, 3) == 32) then
								giveWeapon(hitElement, 32, 30*accept2[weap])
							end
						end
					end
					weap = "ak-47"
					if(accept2[weap] == true) then
						if(accept[weap] == true) then
							giveWeapon(hitElement, 30, 30*accept2[weap])
						else
							if (getPedWeapon (hitElement, 5) == 30) then
								giveWeapon(hitElement, 30, 30*accept2[weap])
							elseif (getPedWeapon (hitElement, 5) == 31) then
								giveWeapon(hitElement, 31, 30*accept2[weap])
							end
						end
					end
					--weap = "m4"
					...
					if(accept2[weap] == true) then
						if(accept[weap] == true) then
							giveWeapon(hitElement, 33, 1*accept2[weap])
						else
							if (getPedWeapon (hitElement, 6) == 33) then
								giveWeapon(hitElement, 33, 1*accept2[weap])
							end
						end
					end
...
if getElementInterior ( source ) == 0 and getElementDimension ( source ) == 0 then
	if(typ == "medicine") then
		...
		if(money < 50) then outputChatBox("Du benötigst "..preis["heilung"].."$!", source, 150, 0, 0) return end
	if laGetElementData ( source, "bestellenhandy" ) == true then outputChatBox("Du hast gerade etwas bestellt, bitte warte diese Lieferung zuerst ab!", source, 150, 0, 0) return end
		laSetElementData ( source, "bankmoney", money - 50 )
		laSetElementData ( source, "bestellenhandy", true )
		timeOutBestellung = setTimer(timeOut_Handy, 10000, 1, source)
	...
	elseif(typ == "backgroundupdate") then
		...
		if(result and numrows > 0) then
			dbFree(query)
			outputDebugString("Background "..text.." fuer "..getPlayerName(source).." gespeichert")
		else
			error("Can't update memo for user "..getPlayerName(source).." in galaxy table")
		end
