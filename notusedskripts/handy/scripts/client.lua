...
function show_browsersite(name)
	...
	elseif(name == "bank") then
		...
		width, height = 120/1.5, 100/1.5
		width, height = width/1920*sx, height/1920*sx
		button["browser:bank_account_label"] = guiCreateLabel(x+addx, y-addy, width, height, "Dein Geld:\n"..bankgeld.."$", false)
	...
	elseif(name == "ammunation") then
		...
		local function updateWeaponPrice()
			...
			local weap = "9mm"
			if(accept2[weap]) then price = price+ammu_select[weap]["ammoprice"]*accept2[weap] end
			weap = "deagle"
			if(accept2[weap]) then price = price+ammu_select[weap]["ammoprice"]*accept2[weap] end
			weap = "shotgun"
			if(accept2[weap]) then price = price+ammu_select[weap]["ammoprice"]*accept2[weap] end
		...
		addEventHandler("onClientGUIClick", this, function()
			...
			if(accept["9mm"]) and (accept2["9mm"] < 1) or (accept["deagle"]) and (accept2["deagle"] < 1) or (accept["shotgun"]) and (accept2["shotgun"] < 1) or (accept["mp5"]) and (accept2["mp5"] < 1) or (accept["ak-47"]) and (accept2["ak-47"] < 1) or (accept["rifle"]) and (accept2["rifle"] < 0) then outputChatBox("Munition auswählen!", 255, 0, 0) return end
...
addCommandHandler("handyinfo", function() outputChatBox("Galaxy SII erstellt von Noneatme!", 0, 200, 200) end)
