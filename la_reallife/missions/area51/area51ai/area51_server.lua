...
function greenGooFadeIn ( player )
	if player then
		...
		if laGetElementData ( player, "thetruthisoutthere_achiev" ) ~= "done" then
			laSetElementData ( player, "thetruthisoutthere_achiev", "done" )
			laSetElementData ( player, "money", laGetElementData ( player, "money" ) + 1500 )
			givePlayerMoney ( player, 1500 )
			triggerClientEvent ( player, "HudEinblendenMoney", getRootElement() )
			playSoundFrontEnd ( player, 40 )
			triggerClientEvent ( player, "showAchievmentBox", player, " The Truth is\n out there!", 15, 10000 )
			
			MySQL_SetString("achievments", "TheTruthIsOutThere", laGetElementData ( player, "thetruthisoutthere_achiev" ), "Name LIKE '"..getPlayerName(player).."'")
		...
		elseif laGetElementData ( player, "silentassasin_achiev" ) ~= "done" and tonumber ( laGetElementData ( player, "alerts" ) ) == 0 then
			laSetElementData ( player, "money", laGetElementData ( player, "money" ) + 1500 )
			givePlayerMoney ( player, 1500 )
			triggerClientEvent ( player, "HudEinblendenMoney", getRootElement() )
			laSetElementData ( player, "silentassasin_achiev", "done" )
			triggerClientEvent ( player, "showAchievmentBox", player, " Silent\n Assasin", 25, 10000 )
			
			MySQL_SetString("achievments", "SilentAssasin", laGetElementData ( player, "silentassasin_achiev" ), "Name LIKE '"..getPlayerName(player).."'")
			...
			laSetElementData ( player, "money", laGetElementData ( player, "money" ) + 250 )
			givePlayerMoney ( player, 250 )
			triggerClientEvent ( player, "HudEinblendenMoney", getRootElement() )
			outputChatBox ( "Da du diese Mission bereits geschafft hast, erhälst du 250$ Belohnung!", player, 0, 125, 0 )
		end
...
function dimensionFixArea51_func ( player )
	...
	if player == client then
		setElementDimension ( player, tonumber ( laGetElementData ( player, "playerid" ) ) + 1 )
...
function startArea51Mission ( player )
	...
	laSetElementData ( player, "isInArea51Mission", true )
	laSetElementData ( player, "alerts", 0 )
	matchinDim = tonumber ( laGetElementData ( player, "playerid" ) ) + 1
...
function equipBotAgain_func ( bot, player )
	...
	if player == client then
		...
		laSetElementData ( player, "alerts", 1 )
	end
end
...
function killPed_func ( bot )
	...
	if laGetElementData ( client, "isInArea51Mission" ) then
		killPed ( bot )
	end
end
