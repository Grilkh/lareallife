function keybind ( vehicle, seat )								--- Tasten festlegen
	...
	if seat == 1 then
		laSetElementData ( vehicle, "seat1", 0 )
	end
	if seat == 2 then
		laSetElementData ( vehicle, "seat2", 0 )
	end
	if seat == 3 then
		laSetElementData ( vehicle, "seat3", 0 )
	end
...
function helidriveby ( source )									--- dranhängen
	...
	laSetElementData ( source, "playervehicle", getPedOccupiedVehicle ( source ) )
	laSetElementData ( source, "oldvehicle", getPedOccupiedVehicle ( source ) )
	...
	if ( getElementData ( source, "playervehicle" ) ) then 
		if ( getElementModel ( getElementData ( source, "playervehicle" ) ) ) == 487 or ( getElementModel ( getElementData ( source, "playervehicle" ) ) ) == 497 then
			laSetElementData ( source, "seat", ( getPedOccupiedVehicleSeat ( source ) ) )
			if ( getElementData ( source, "seat" ) ) == 2 then 
				...
				laSetElementData ( source, "oldseat", 2)
				laSetElementData ( playervehicle, "seat2", 1 )
			else
				if ( getElementData ( source, "seat" ) ) == 3 then
					removePedFromVehicle ( source )
					attachElements ( source, getElementData ( source, "playervehicle" ) , 1.2, 0.3, 0 )
					setPedWeaponSlot( source, 5 )
					laSetElementData ( source, "oldseat", 3)
					laSetElementData ( playervehicle, "seat3", 1 )
				else
					if ( getElementData ( source, "seat" ) ) == 1 then
						removePedFromVehicle ( source )
						attachElements ( source, getElementData ( source, "playervehicle" ) , 1.2, 1.7, 0 )
						setPedWeaponSlot( source, 5 )
						laSetElementData ( source, "oldseat", 1)
						laSetElementData ( playervehicle, "seat1", 1 )
...
function rein ( source )										--- Wieder einsteigen
    ...
    if ( getElementData ( source, "oldvehicle" ) ) ~= 0 then
		...
		if ( getElementData ( source, "oldseat") == 1) then
			laSetElementData ( getElementData ( source, "playervehicle" ), "seat 1", 0 )
		end
		if ( getElementData ( source, "oldseat") == 2) then
			laSetElementData ( getElementData ( source, "playervehicle" ), "seat 2", 0 )
		end
		if ( getElementData ( source, "oldseat") == 3) then
			laSetElementData ( getElementData ( source, "playervehicle" ), "seat 3", 0 )
		end
		laSetElementData ( source, "oldseat", 0)
		laSetElementData ( source, "oldvehicle", 0)
...
function eject ( source )										--- Abspringen
	...
	if getElementData ( source, "oldseat") == 1 then
		laSetElementData ( veh, "seat1", 0 )
	end
	if getElementData ( source, "oldseat") == 2 then
		laSetElementData ( veh, "seat2", 0 )
	end
	if getElementData ( source, "oldseat") == 3 then
		laSetElementData ( veh, "seat3", 0 )
	end
	laSetElementData ( source, "oldseat", 0 )
	laSetElementData ( source, "oldvehicle", 0 )
	unbindKey ( source, "mouse_wheel_up", "down", weaponsup )
...
function helidestroyed ()
	...
	if getElementData ( source, "seat1" ) == 1 or getElementData ( source, "seat2" ) == 1 or getElementData ( source, "seat3" ) == 1 then
		laSetElementData ( source, "seat1", 0 )
		laSetElementData ( source, "seat2", 0 )
		laSetElementData ( source, "seat3", 0 )
	end
end
