...
function buyItGiveItem ( typ, pname, anzahl, id, offerer )
	...
	if typ == "Drug" then
		if isElement ( ingame ) and laGetElementData ( ingame, "loggedin" ) and laGetElementData ( ingame, "loggedin" ) == 1 then
			laSetElementData ( ingame, "drugs", laGetElementData ( ingame, "drugs" ) + anzahl )
		else
	...
	elseif typ == "Mats" then
		if isElement ( ingame ) and laGetElementData ( ingame, "loggedin" ) and laGetElementData ( ingame, "loggedin" ) == 1 then
			laSetElementData ( ingame, "mats", laGetElementData ( ingame, "mats" ) + anzahl )
		else
	...
	elseif typ == "Houses" then
		...
		if isElement ( getPlayerFromName ( pname ) ) then
			laSetElementData ( getPlayerFromName ( pname ), "housekey", key )
		end
		if isElement ( getPlayerFromName ( offerer ) ) then
			laSetElementData ( getPlayerFromName ( offerer ), "housekey", 0 )
		end
	elseif typ == "Special" then
		...
		sendBuyItMessage ( "[LA]Leyynen", pname.." hat eine Sonderauktion gewonnen." )
	end
end
...
function makeOffer_func ( typ, startGebot, description, timeToRun, count )
	...
	if typ and description and startGebot and tonumber ( timeToRun ) and ( tonumber ( count ) or count == nil ) then
		...
		elseif typ == "Mats" then
			...
			if mats >= count then
				laSetElementData ( player, "mats", mats - count )
				outputChatBox ( "Gebot eingestellt!", player, 0, 125, 0 )
			else
				validValues = false
				outputChatBox ( "Du hast nicht genug Materialien dafür!", player, 125, 0, 0 )
			end
		elseif typ == "Drug" then
			local drugs = laGetElementData ( player, "drugs" )
			if drugs >= count then
				laSetElementData ( player, "drugs", drugs - count )
				outputChatBox ( "Gebot eingestellt!", player, 0, 125, 0 )
			else
				validValues = false
		...
		elseif typ == "Houses" then
			outputChatBox ( "Vorruebergehend deaktiviert.", player, 125, 0, 0 )
			return true
			--[[if laGetElementData ( lp, "housekey" ) > 0 then
		...
		elseif typ == "Prestige" then
			...
			outputChatBox ( "Nicht verfügbar.", player, 125, 0, 0 )
			count = 0
		end
		if validValues then
			_G["cur"..typ.."Offers"] = _G["cur"..typ.."Offers"] + 1
	...
	if _G[typ.."Offers"][id]["LaeuftBis"] then
		...
		if curGebot < gebot then
			if not MySQL_DatasetExist("buyit", "Hoechstbietender LIKE '"..pname.."' AND Typ LIKE '"..typ.."'") then
				if laGetElementData ( player, "bankmoney" ) >= gebot then
					local betOK = true
					if typ == "Veh" then
						local carslot = buyItGetFreeCarslot ( pname )
						if carslot then
							laSetElementData ( player, "carslot"..carslot, 3 )
						else
							betOK = false
					...
					if betOK then
						...
						if target then
							if typ == "Veh" then
								for i = 1, 10 do
									if laGetElementData ( target, "carslot"..i ) == 3 then
										laSetElementData ( target, "carslot"..i, 0 )
										break
									end
								end
							end
							outputChatBox ( "Du wurdest soeben von "..pname.." ueberboten!", target, 125, 0, 0 )
							laSetElementData ( target, "bankmoney", laGetElementData ( target, "bankmoney" ) + curGebot )
						elseif hoechstbietender ~= "-" then
							...
							buyItSendMail ( hoechstbietender.."@LA-RL.com", "Du wurdest bei einer Auktion von "..pname.." ueberboten!" )
						end
						-- Aktueller Hoechstbietender --
						laSetElementData ( player, "bankmoney", laGetElementData ( player, "bankmoney" ) - gebot )
						outputChatBox ( "Du hast dein Gebot erfolgreich abgegeben und bist nun Höchstbietender!", player, 0, 125, 0 )
						outputChatBox ( "Wir werden dich über den weiteren Verlauf der Auktion informieren!", player, 0, 125, 0 )
				...
				outputChatBox ( "Du kannst nur auf einen Artikel der selben Kategorie zur gleichen Zeit bieten.", player, 125, 0, 0 )
			end
		else
