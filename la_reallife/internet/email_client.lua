...
function showEmailRecieveCenter_func ()
	...
	if gWindow["email"] then
		...
		false )
		gButton["delEmail"] = guiCreateButton(9+(67+10)*2,297,67,40,"Löschen",false,gWindow["email"])
...
function deleteMail ( i )
	...
	local mailList = xmlLoadFile ( ":"..resourceName.."/internet/mails.xml" )
...
function reciveMail_func ( newMail )
	...
	outputChatBox ( "Dein Posteingang ist voll, deine E-Mail wurde gelöscht.", 125, 0, 0 )
end
...
function loadMails ()
	...
	if not mailList then
		local mailList = xmlCreateFile ( ":"..resourceName.."/internet/mails.xml", "mails" )
...
function showEmailSpecialWindow ( write, i, from, betreff, text )
	...
	if gWindow["emailWindow"] then
		...
		false )
		gButton[3] = guiCreateButton(285,231,75,41,"Löschen",false,gWindow["emailWindow"])
