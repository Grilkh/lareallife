...
function showAdminMenue ()
	...
	if gWindow["plistadmin"] then
		...
		gWindow["plistadmin"] = guiCreateWindow(screenwidth/2-568/2,120,568,585,"Adminmenü",false)
			...
			function ()
				if source == gImage[1] or source == gLabel[1] then
					if guiCheckBoxGetSelected ( gCheck[1] ) then
						...
						outputChatBox ( "Bitte aktiviere das Kontrollkästchen!", 125, 0, 0 )
					end
				end
			end
		)
		...
		addEventHandler ( "onClientGUIClick", gButton[3], 
			function ()
				row, column = guiGridListGetSelectedItem(gGrid["plistadmin"])
				player = guiGridListGetItemText ( gGrid["plistadmin"], row, column )
				triggerServerEvent ( "executeAdminServerCMD", lp, "warn", player.." "..guiGetText ( gMemo[1] ) )
			end,
		false )
		...
		gLabel[4] = guiCreateLabel(264,202,48,17,"Stunden",false,gTab[1])
		...
		addEventHandler ( "onClientGUIClick", gButton[5], 
			function ()
				triggerServerEvent ( "executeAdminServerCMD", lp, "runban", guiGetText ( gEdit[2] ) )
			end,
		false )
		...
		gTab[2] = guiCreateTab("Räumlich",gTabPanel["adminMenue"])
		...
		addEventHandler ( "onClientGUIClick", gButton[14], 
			function ()
				...
				if guiCheckBoxGetSelected ( gCheck[2] ) then
					triggerServerEvent ( "executeAdminServerCMD", lp, "rslap", player.." ja" )
				else
					triggerServerEvent ( "executeAdminServerCMD", lp, "rslap", player )
				end
			end,
		false )
		...
		false )
		gButton[17] = guiCreateButton(8,275,78,38,"Passwort ändern",false,gTab[3])
		...
		gCheck[2] = guiCreateCheckBox(86,151,92,21,"Anzünden?",false,false,gTab[3])
		...
		gButton[18] = guiCreateButton(6,21,84,46,"Query ausführen",false,gTab[4])
		...
		gTab[5] = guiCreateTab("Chat",gTabPanel["adminMenü"])
		...
		false )
		gButton[25] = guiCreateButton(6,122,82,41,"Flüstern",false,gTab[5])
