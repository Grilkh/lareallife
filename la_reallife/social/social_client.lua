...
socialNeeds = {
 ...
 ["God of LA"]="30 Stunden Spielzeit\nAlle 25 Paeckchen\nEigene Yacht\n250.000 $\nEigenes Auto",
...
function getPlayerSocialAvailableStates ()
	...
	-- **Gruppe 7 - Suchtbasiert** --
		if laGetElementData ( player, "drugAddictPoints" ) and laGetElementData ( player, "alcoholAddictPoints" ) and laGetElementData ( player, "cigarettAddictPoints" ) then
			if math.floor ( laGetElementData ( player, "cigarettAddictPoints" ) / addictLevelDivisors[1] ) >= 3 then
				availableSocialStates[20] = true
			elseif math.floor ( laGetElementData ( player, "alcoholAddictPoints" ) / addictLevelDivisors[2] ) >= 3 then
				availableSocialStates[21] = true
			elseif math.floor ( laGetElementData ( player, "drugAddictPoints" ) / addictLevelDivisors[3] ) >= 3 then
				availableSocialStates[22] = true
			end
		end
	-- **Gruppe 8 - Hobbybasiert** --
--[[		if laGetElementData ( player, "club" ) == "rc" then
			availableSocialStates[26] = true
		elseif laGetElementData ( player, "club" ) == "gartenverein" then
	...
	if getElementData ( lp, "loggedin" ) == 1 then
		...
		for key, state in pairs ( availableSocialStates ) do
			if not oldAvailableSocialStates[key] and availableSocialStates[key] then
				...
				outputChatBox ( "Vergiss nicht, ihn im Optionsmenü zu aktivieren!", 0, 125, 0 )
			end
		end
