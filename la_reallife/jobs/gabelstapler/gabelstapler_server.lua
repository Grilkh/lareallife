...
if getElementType(hitElement) == "player" and (dim) then
	if isPedInVehicle ( hitElement ) == false then
		...
		if laGetElementData (hitElement, "job") == "gabelstapler" then
...
addEventHandler("onGabelstaplerJobStart", getRootElement(), function()
	...
	outputChatBox("[AUFGABE]: Für jede Kiste bekommst du "..lohn.."$. Du bekommst zusätzliches Geld für die grünen Kisten.", thePlayer, 0, 100, 200)
		...
		setTimer(function()
			...
			setTimer(function()
				if(isElement(thePlayer)) then
					...
					laSetElementData(thePlayer, "job", "gabelstapler")
					...
					setElementVisibleTo ( gabelstaplerblip, thePlayer, true )
...
function GabelstaplerStop ( thePlayer )
	--local thePlayer = source
	--if(stabler[thePlayer]) and (getElementData(thePlayer, "IsInGabelJob") == true) then
...
addEventHandler("onGabelstaplerJobAbgeliefert", getRootElement(), function(lohn2)
	...
	laSetElementData (source, "money", money + gewinn) 
