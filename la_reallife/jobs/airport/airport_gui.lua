...
function showAirportJobGui_func ()
	...
	if gWindow["airportJob"] then
		...
		gWindow["airportJob"] = guiCreateWindow(screenwidth/2-550/2,screenheight/2-300/2,550,300,"Flughafenmitarbeiter",false)
		guiSetAlpha(gWindow["airportJob"],1)

		gGrid["airportChoose"] = guiCreateGridList(0.0189,0.3154,0.3025,0.723,true,gWindow["airportJob"])
		guiGridListSetSelectionMode(gGrid["airportChoose"],2)
		gColumn["job"] = guiGridListAddColumn(gGrid["airportChoose"],"Auftrag",0.65)
		gColumn["belohnung"] = guiGridListAddColumn(gGrid["airportChoose"],"$",0.25)
		...
		guiGridListSetItemText ( gGrid["airportChoose"], row, gColumn["belohnung"], "100$", false, false )
		local row = guiGridListAddRow ( gGrid["airportChoose"] )
		guiGridListSetItemText ( gGrid["airportChoose"], row, gColumn["job"], "Insektenkiller", false, false )
		guiGridListSetItemText ( gGrid["airportChoose"], row, gColumn["belohnung"], "250$", false, false )
		local row = guiGridListAddRow ( gGrid["airportChoose"] )
		guiGridListSetItemText ( gGrid["airportChoose"], row, gColumn["job"], "Leichter Flug", false, false )
		guiGridListSetItemText ( gGrid["airportChoose"], row, gColumn["belohnung"], "400$", false, false )
		local row = guiGridListAddRow ( gGrid["airportChoose"] )
		guiGridListSetItemText ( gGrid["airportChoose"], row, gColumn["job"], "Mittlerer Flug", false, false )
		guiGridListSetItemText ( gGrid["airportChoose"], row, gColumn["belohnung"], "600$", false, false )
		local row = guiGridListAddRow ( gGrid["airportChoose"] )
		guiGridListSetItemText ( gGrid["airportChoose"], row, gColumn["job"], "Schwerer Flug", false, false )
		guiGridListSetItemText ( gGrid["airportChoose"], row, gColumn["belohnung"], "1000$", false, false )

		gLabel["airportInfo1"] = guiCreateLabel(0.0231,0.1039,0.6849,0.1792,"Herzlich willkommen am Los Angeles International Airport!\n\nBitte wähle deinen Auftrag aus!",true,gWindow["airportJob"])
		...
		gLabel["airportInfo2"] = guiCreateLabel(0.334,0.3799,0.3592,0.3,"Als Kofferpacker ist es\ndeine Aufgabe, das Gepäck\nzu den Flugzeugen zu bringen.",true,gWindow["airportJob"])
		...
		gLabel["airportInfo3"] = guiCreateLabel(0.3739,0.2867,0.2836,0.0968,"Kofferpacker",true,gWindow["airportJob"])
		...
		addEventHandler("onClientGUIClick", getRootElement(),
			function ()
				...
				elseif source == gButton["airportTakeJob"] then
					...
					if labeltext == "Kofferpacker" then
						if getElementData ( localPlayer, "carlicense" ) == 1 then
							hideAllMarkersAirport()
							triggerServerEvent ( "airportjobDimFix", getLocalPlayer(), getLocalPlayer() )
							setElementPosition ( localPlayer, -1262.0727539063, 33, 13.84 )
							guiSetVisible ( gWindow["airportJob"], false )
							showCursor(false)
							triggerServerEvent ( "cancel_gui_server", getLocalPlayer() )
							count = 0
					...
					elseif labeltext == "Insektenkiller" then
						if tonumber ( getElementData ( localPlayer, "airportlvl" ) ) >= 5 then
							if getElementData ( localPlayer, "planelicensea" ) == 1 then
								hideAllMarkersAirport()
								triggerServerEvent ( "airportJobInsektenkiller", getLocalPlayer(), getLocalPlayer() )
								guiSetVisible ( gWindow["airportJob"], false )
								showCursor(false)
								triggerServerEvent ( "cancel_gui_server", getLocalPlayer() )
								count = 0
							...
							outputChatBox ( "Dein Flughafen-Level ist nicht hoch genug - erst ab Level 5 verfügbar!", 125, 0, 0 )
						end
					elseif labeltext == "Leichter Flug" then
						if tonumber ( getElementData ( localPlayer, "airportlvl" ) ) >= 10 then
							if getElementData ( localPlayer, "planelicensea" )  == 1 then
								...
								elseif guiRadioButtonGetSelected ( gRadio["beagle"] ) then
									...
									outputChatBox ( "Bitte wähle einen gültigen Flugzeugtypen aus!", 125, 0, 0 )
								end
							else
								outputChatBox ( "Du braucht einen Flugschein Klasse A!", 125, 0, 0 )
							end
						else
							outputChatBox ( "Dein Flughafen-Level ist nicht hoch genug - erst ab Level 10 verfügbar!", 125, 0, 0 )
						end
					elseif labeltext == "Mittlerer Flug" then
						if tonumber ( getElementData ( localPlayer, "airportlvl" ) ) >= 15 then
							...
							elseif guiRadioButtonGetSelected ( gRadio["shamal"] ) then
								if getElementData ( localPlayer, "planelicenseb" )  == 1 then
							...
							outputChatBox ( "Dein Flughafen-Level ist nicht hoch genug - erst ab Level 15 verfügbar!", 125, 0, 0 )
						end
					elseif labeltext == "Schwerer Flug" then
						if tonumber ( getElementData ( localPlayer, "airportlvl" ) ) >= 20 then
							if getElementData ( localPlayer, "planelicenseb" )  == 1 then
							...
							outputChatBox ( "Dein Flughafen-Level ist nicht hoch genug - erst ab Level 20 verfügbar!", 125, 0, 0 )
						end
					end
				else
		...
		gLabel["airportInfo4"] = guiCreateLabel(0.3361,0.7,0.292,0.0717,"Aktuelles Flughafenlevel:",true,gWindow["airportJob"])
		...
		gLabel["airportInfo7"] = guiCreateLabel(0.7878,0.2007,0.2055,0.0753,"Baggage",true,gWindow["airportJob"])
	...
	guiSetText ( gLabel["airportInfo5"], getElementData ( localPlayer, "airportlvl" ) )
end
...
function showDeliverPoints()
	...
	local dim = tonumber ( getElementData ( localPlayer, "playerid" ) ) + 1
	hideDeliverPoints()
	BaggageUnload1Vehicle = createVehicle ( 577, -1338.7501220703, -223.37954711914, 14.209453582764, 0, 0, 134.99987792969 )
	BaggageUnload1 = createMarker ( -1361.6362304688, -228.25860595703, 14.143964767456, "checkpoint", 10, 255, 255, 255, 150, getRootElement() )
	BaggageUnload2Vehicle = createVehicle ( 592, -1501.3070068359, -154.43298339844, 15, 0, 0, 135 )
	BaggageUnload2 = createMarker ( -1522.2647705078, -160.74020385742, 14.1484375, "checkpoint", 10, 255, 255, 255, 150, getRootElement() )
	BaggageUnload3Vehicle = createVehicle ( 417, -1221.8127441406, -8.256688117981, 14.623241424561 )
	BaggageUnload3 = createMarker ( -1234.8166503906, -12.079794883728, 14.1484375, "checkpoint", 10, 255, 255, 255, 150, getRootElement() )
	...
	BaggageUnload1Blip = createBlip ( -1361.6362304688, -228.25860595703, 14.143964767456, 0, 2, 255, 0, 0, 255, 0, 99999.0, getLocalPlayer() )
	BaggageUnload2Blip = createBlip ( -1522.2647705078, -160.74020385742, 14.1484375, 0, 2, 255, 0, 0, 255, 0, 99999.0, getLocalPlayer() )
	BaggageUnload3Blip = createBlip ( -1234.8166503906, -12.079794883728, 14.1484375, 0, 2, 255, 0, 0, 255, 0, 99999.0, getLocalPlayer() )
...
function BaggageUnloadMarkerHit ( hit, dim )
	...
	if dim and hit == localPlayer then
		if source == BaggageUnload1 or source == BaggageUnload2 or source == BaggageUnload3 then
			...
			if trailer then
				...
				count = count + 1
				if count == 3 then
					triggerServerEvent ( "baggageMissionComplete", getRootElement(), getLocalPlayer() )
				else
					outputChatBox ( "[AUFGABE]: Hol dir einen neuen Anhänger!", 0, 100, 200 )
				end
				playSoundFrontEnd ( 43 )
				triggerServerEvent ( "killTrailer", getLocalPlayer(), getLocalPlayer(), trailer )
			end
		end
	end
end
...
function CropDusterMarkerHit ( hit, dim )
	...
	if hit == getLocalPlayer() and dim then
		if source == CropdusterMarker1 or source == CropdusterMarker2 or source == CropdusterMarker3 or source == CropdusterMarker4 or source == CropdusterMarker5 then
			...
			count = count + 1
			if count == 5 then
				triggerServerEvent ( "cropdusterMissionComplete", getRootElement(), getLocalPlayer() )
			end
			playSoundFrontEnd ( 43 )
		end
	end
...
function showCropdusterPoints ()
	...
	CropdusterMarker1  = createMarker ( -1104.9104003906, -978.28009033203, 189.46875, "ring", 10, 255, 255, 255, 150, getRootElement() )
	CropdusterMarker2  = createMarker ( -281.89483642578, -1521.7196044922, 89.01439666748, "ring", 10, 255, 255, 255, 150, getRootElement() )
	CropdusterMarker3  = createMarker ( -263.64831542969, -1369.1451416016, 77.448867797852, "ring", 10, 255, 255, 255, 150, getRootElement() )
	CropdusterMarker4  = createMarker ( -279.80813598633, -957.28393554688, 127.56575012207, "ring", 10, 255, 255, 255, 150, getRootElement() )
	CropdusterMarker5  = createMarker ( -480.54431152344, -1359.1550292969, 93.225845336914, "ring", 10, 255, 255, 255, 150, getRootElement() )
	...
	CropdusterMarker1Blip = createBlip ( -1104.9104003906, -978.28009033203, 189.46875, 0, 2, 255, 0, 0, 255, 0, 99999.0, getLocalPlayer() )
	CropdusterMarker2Blip = createBlip ( -281.89483642578, -1521.7196044922, 89.01439666748, 0, 2, 255, 0, 0, 255, 0, 99999.0, getLocalPlayer() )
	CropdusterMarker3Blip = createBlip ( -263.64831542969, -1369.1451416016, 77.448867797852, 0, 2, 255, 0, 0, 255, 0, 99999.0, getLocalPlayer() )
	CropdusterMarker4Blip = createBlip ( -279.80813598633, -957.28393554688, 127.56575012207, 0, 2, 255, 0, 0, 255, 0, 99999.0, getLocalPlayer() )
	CropdusterMarker5Blip = createBlip ( -480.54431152344, -1359.1550292969, 93.225845336914, 0, 2, 255, 0, 0, 255, 0, 99999.0, getLocalPlayer() )
	...
	outputChatBox ( "[AUFGABE]: Verteile die Pestiziede über den Feldern!", 0, 100, 200 )
end
...
function startFreightMission ( veh )
	...
	if rnd == 1 then		-- Schrottplatz
		AirportTargetMarker = createMarker ( 393.78289794922, 2502.5717773438, 15.734373092651, "ring", 10, 255, 255, 255, 150, getRootElement() )
		AirportTargetBlip = createBlip ( 393.78289794922, 2502.5717773438, 15.734373092651, 0, 2, 255, 0, 0, 255, 0, 99999.0, getLocalPlayer() )
		outputChatBox ( "[AUFGABE]: Fliege dein Flugzeug unbeschädigt zum Flugzeugfriedhof.", 0, 100, 200 )
	elseif rnd == 2 then 	-- LV
		AirportTargetMarker = createMarker ( 1433.0295410156, 1463.6586914063, 10.8203125, "ring", 10, 255, 255, 255, 150, getRootElement() )
		AirportTargetBlip = createBlip ( 1433.0295410156, 1463.6586914063, 10.8203125, 0, 2, 255, 0, 0, 255, 0, 99999.0, getLocalPlayer() )
		outputChatBox ( "[AUFGABE]: Fliege dein Flugzeug unbeschädigt zum Las Venturas International Airport.", 0, 100, 200 )
	else					-- LS
		AirportTargetMarker = createMarker ( 1834.8217773438, -2494.7470703125, 13.554689407349, "ring", 10, 255, 255, 255, 150, getRootElement() )
		AirportTargetBlip = createBlip ( 1834.8217773438, -2494.7470703125, 13.554689407349, 0, 2, 255, 0, 0, 255, 0, 99999.0, getLocalPlayer() )
		outputChatBox ( "[AUFGABE]: Fliege dein Flugzeug unbeschädigt zum Los Santos International Airport.", 0, 100, 200 )
	end
	...
	showCursor(false)
	triggerServerEvent ( "cancel_gui_server", getLocalPlayer() )
	triggerServerEvent ( "airportJobFlight", getLocalPlayer(), getLocalPlayer(), veh, x, y, z, rot )
end
		...
		if source == AirportTargetMarker then
			if hit == localPlayer then
				if freightModel[ getElementModel ( getPedOccupiedVehicle ( lp ) ) ] then
					...
					triggerServerEvent ( "airportJobFreightFinished", getLocalPlayer(), getLocalPlayer(), vehid )
					playSoundFrontEnd ( 43 )
				end
			end
		end
