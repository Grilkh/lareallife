			...
			if(getElementType(hitElement) == "player") then
				if isEvil(hitElement) then
					if (doingbankrob[hitElement] == true) then
						...
						setTimer(function()
							...
							local money = laGetElementData (hitElement, "money")
							laSetElementData (hitElement, "money", money + geld)
				...
				elseif isOnStateDuty(hitElement) then
					...
					setTimer(function()
						...
						laSetElementData ( hitElement, "boni", laGetElementData ( hitElement, "boni" ) + (geld/20) )
...
addEventHandler("onPlayerWasted", getRootElement(), function()
	if(doingbankrob[source] == true) then
		if(erbeutetbankrob[source]) then
			...
			local lostmoney
			if newmoney >= 0 then
				laSetElementData(source, "money", newmoney)
				lostmoney = geld
			else
...
function defusebombend(player)
	if not isPedDead ( player ) and getElementData ( player, "loggedin" ) == 1 then
		if defusestate == "machend" then
			...
			local geld = math.random(5000, 10000)
			laSetElementData ( player, "boni", laGetElementData ( player, "boni" ) + (geld/10) )
