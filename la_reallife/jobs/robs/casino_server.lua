...
if getElementType(hitElement) == "player" and (dim) then
	if isPedInVehicle ( hitElement ) == false then
		if isEvil( hitElement ) then
			...
			laSetElementData ( hitElement, "isinRob", true )
...
if getElementType(hitElement) == "player" and (dim) then
	if isPedInVehicle ( hitElement ) == false then
		...
		laSetElementData ( hitElement, "isinRob", false )
		if isEvil(hitElement) then
		...
		if getElementType(hitElement) == "player" and (dim) then
			if robstate == "ausstehend" then
				if isEvil( hitElement ) then
					...
					laSetElementData ( hitElement, "money", laGetElementData ( hitElement, "money" ) + pricebrutto )
					givePlayerMoney ( hitElement, pricebrutto )
					...
					anzahlState = 0
					local fraktion = laGetElementData(hitElement, "fraktion")
					...
					laSetElementData ( hitElement, "boni", laGetElementData ( hitElement, "boni" ) + (pricebrutto/200) )
...
if pdim == casinodim then
	...
	isElementWithinMarker(player, defusemarker) then
		if isOnStateDuty(player) then
			if not isPedDead ( player ) and laGetElementData ( player, "loggedin" ) == 1 then
				defusestate = "machend"
...
function sohier(player)
	if not isPedDead ( player ) and getElementData ( player, "loggedin" ) == 1 then
		if defusestate == "machend" then
			...
			laSetElementData ( player, "boni", laGetElementData ( player, "boni" ) + (pricebrutto/100) )
