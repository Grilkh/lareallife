...
local geld_minimal, geld_maximal = 1000, 3000

local robfaction 

local koordinaten = {
	[2] = ""..camorrax..", "..camorray..", "..camorraz.."", -- Camorra
	[3] = ""..yakuzax..", "..yakuzay..", "..yakuzaz.."", -- Yakuza
	[7] = ""..surenosx..", "..surenosy..", "..surenosz.."", -- Surenos
	[9] = ""..comptonx..", "..comptony..", "..comptonz.."", -- Compton Families
	[10] = ""..wattsx..", "..wattsy..", "..wattsz.."" -- Watts Soldiers
}
...
var["element:blip"] = createBlip(1426.3249511719, -1307.8471679688, 12.558536529541, 19, 2, 255, 0, 0, 255, 0, 500)
...
var["element:marker1"] = createMarker(1425.4056396484, -1308.5848388672, 13.7546875, "corona", 1.5, 0, 255, 0, 150)
-- Davor: 1424.2760009766, -1308.6253662109, 13.5546875

var["element:marker2"] = createMarker(1062.1541748047, 2077.4145507813, 11.0203125, "corona", 1.5, 0, 255, 0, 150)
setElementDimension(var["element:marker2"], dim)
-- Davor: 1062.1513671875, 2078.7114257813, 10.8203125


addEventHandler("onMarkerHit", var["element:marker1"], function(hitElement, matchdim)
if getElementType(hitElement) == "player" and (matchdim) then
	if isPedInVehicle ( hitElement ) == false then
			setElementPosition(hitElement, 1062.1513671875, 2078.7114257813, 10.8203125)
			setElementDimension(hitElement, dim)
			if(isEvil(hitElement)) and (running == false) then
				outputChatBox("[AUFGABE]: Finde und erpresse den Manager, um an die Waffen zu kommen!", hitElement, 0, 100, 200)
			end
		end
	end
end)

addEventHandler("onMarkerHit", var["element:marker2"], function(hitElement, matchdim)
if getElementType(hitElement) == "player" and (matchdim) then
	if isPedInVehicle ( hitElement ) == false then
			setElementPosition(hitElement, 1424.2760009766, -1308.6253662109, 13.5546875)
	...
	if(isElement(kiste[id])) and (isElement(truck)) then
		...
		if(getVehicleOccupant(truck)) then
			if((currentkiste-1) > 0) then
				outputChatBox("[INFO]: Du hast eine Kiste verloren! Sei vorsichtig, du hast nur noch "..(currentkiste-1)..".", getVehicleOccupant(truck), 150, 0, 0)
			else
				outputChatBox("[INFO]: Du hast alle deine Kisten verloren!", getVehicleOccupant(truck), 175, 0, 0)
			end
		end
	end
end
...
local function setPlayerToUeberfall(thePlayer)
	local msg = "[ILLEGAL]: Das Warenhaus wird überfallen!"
	local fraktion = laGetElementData (thePlayer, "fraktion")
	robfaction = fraktion
	...
	outputLog ( "[WARENHAUS]: "..getPlayerName(player).." ("..robfaction..") ueberfaellt das Warenhaus!", "bad")
	...
	outputChatBox("[AUFGABE]: Fahre den Laster zu deiner Basis! Pass auf, dass du keine Kisten verlierst, die oben draufgeschnallt wurden!", thePlayer, 0, 100, 200)
	-- MARKER --
	-- SICHERHEITSHINWEIS --
	local fraktion = laGetElementData (thePlayer, "fraktion")
	...
	addEventHandler("onMarkerHit", marker, function(hitElement)
		if(hitElement == truck) then
			...
			-- SICHERHEITSHINWEIS --
			local money = laGetElementData (occupant, "money")
			laSetElementData (occupant, "money", money + geld)
			givePlayerMoney(occupant, geld)
			giveWeapon(occupant, 35, 5, true)
			outputChatBox("[INFO]: Du hast erfolgreich "..kisten.." Kisten ins Lager gebracht und "..geld.."$ erbeutet!", occupant, 0, 200, 0)
			outputChatBox("Ausserdem hast du ein Raketenwerfer mit 5 Schuss erhalten.", occupant, 0, 200, 0)
			local msg = "[WARENHAUS]: Das Warenhaus wurde erfolgreich überfallen!"
			sendMSGForFaction ( msg, 1, 200, 0, 0 )
			if robfaction ~= 2 then
				sendMSGForFaction ( msg, 2, 200, 200, 0 )
			end
			...
			sendMSGForFaction ( msg, robfaction, 0, 150, 0 )
			outputLog ( "[WARENHAUS]: "..getPlayerName(occupant).." ("..robfaction..") hat das Warenhaus erfolgreich ueberfallen!", "bad")
			robfaction = nil
			destroyElement(truck)
			destroyElement(blip)
			destroyElement(marker)
	...
	addEventHandler("onVehicleExplode", truck, function()
		local msg = "[WARENHAUS]: Der Warenhauslaster ist explodiert!"
		local fraktion = laGetElementData (thePlayer, "fraktion")
		robfaction = fraktion
		...
		outputLog ( "[WARENHAUS]: Der Wahrenhauslaster ist explodiert!", "bad")
		for index, k in pairs(kiste) do
...
addEventHandler("onWarenhausUeberfallStart", getRootElement(), function()
	...
	if(running == true) then
		outputChatBox("Ein Überfall hat bereits stattgefunden! Du musst noch warten!", source, 175, 0, 0)
	return end
	...
	outputChatBox(getPlayerName(source).." sagt: Hände hoch! Sofort!", source, 255, 255, 255)
	...
	setTimer(function()
		...
		setTimer(function()
			...
			setTimer(function()
				outputChatBox("Manager sagt: Hinter dem Warenhaus, bitte töten Sie mich nicht!", thePlayer, 255, 255, 255)
				setTimer(function()
...
-- SICHERHEITSHINWEIS --
--[[
function isGangmitglied(thePlayer)
	return true;
end]]