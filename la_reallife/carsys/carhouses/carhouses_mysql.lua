MySQLCarhouseNames = {}
 ...
 MySQLCarhouseNames[6] = "Grotti"
 MySQLCarhouseNames[7] = "S und O\nSpecialist"
 MySQLCarhouseNames[8] = "Zero's"
 MySQLCarhouseNames[9] = "Pine Cars"
 MySQLCarhouseNames[10] = "Cable Cars"
...
MySQLCarhouseBlips = {}
 ...
 MySQLCarhouseBlips["Grotti"] = 55
 MySQLCarhouseBlips["S und O\nSpecialist"] = 55
 MySQLCarhouseBlips["Zero's"] = 47
 MySQLCarhouseBlips["Pine Cars"] = 55
 MySQLCarhouseBlips["Cable Cars"] = 55
...
MySQLCarhouseBlipRanges = {}
 ...
 MySQLCarhouseBlipRanges["S und O\nSpecialist"] = 200
 MySQLCarhouseBlipRanges["Grotti"] = 200
 MySQLCarhouseBlipRanges["Zero's"] = 200
 MySQLCarhouseBlipRanges["Pine Cars"] = 200
 MySQLCarhouseBlipRanges["Cable Cars"] = 200
...
function mySQLCarhouseCreate ()
	...
	badMySQLCarHouseXSpawns[sx] = true
	...
	laSetElementData ( MySQLCarhouses[ID]["pickup"], "ID", ID )
	
	addEventHandler ( "onPickupHit", MySQLCarhouses[ID]["pickup"],
		function ( player )
			if getElementType ( player ) == "player" then
				if not laGetElementData ( player, "isInCarHouse" ) then
					if not getPedOccupiedVehicle ( player ) then
						laSetElementData ( player, "isInCarHouse", true )
						MySQLCarhouseEnter ( player, laGetElementData ( source, "ID" ) )
					end
				end
			end
		end
...
function MySQLCarhouseEnter ( player, ID )
	...
	laSetElementData ( player, "lookingAtCar", 1 )
	laSetElementData ( player, "carHouse", ID )
...
function MySQLCarhouseLeft ( player )
	...
	local car = laGetElementData ( player, "lookingAtCar" )
	local id = laGetElementData ( player, "carHouse" )
	local count = MySQLCarhouses[id]["counter"]

	if car == 1 then
		laSetElementData ( player, "lookingAtCar", count )
	else
		laSetElementData ( player, "lookingAtCar", laGetElementData ( player, "lookingAtCar" ) - 1 )
	end
...
function leaveMySQLCarhouseCar ( player )
	...
	laSetElementData ( player, "isInCarHouse", false )
