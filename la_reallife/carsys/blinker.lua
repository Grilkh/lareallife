	...
	for index, vehtable in pairs(blinkertable) do
		...
		if (BlinkerStatus == 0) then	
				if (BremslichtStatus == 0) then
					...
					setPrivVehCorrectLightColor3 (BlinkerCar)
					if (laGetElementData( BlinkerCar, "light") == false) then
						setVehicleLightState ( BlinkerCar, 0, 1 )
						setVehicleLightState ( BlinkerCar, 1, 1 )				
					elseif ((laGetElementData( BlinkerCar, "light") == true)) then
						setVehicleLightState ( BlinkerCar, 0, 0 )
						setVehicleLightState ( BlinkerCar, 1, 0 )
					else
						laSetElementData( BlinkerCar, "light",false)
					end
				end
		-- Linker		
...
--[[function togglelicht( vehinplayer )
	for index, vehtable in pairs(blinkertable) do
		...
		elseif (getPedOccupiedVehicle ( vehinplayer ) == BlinkerCartl)  and (Lichtstatetl == 1) and (BlinkerStatustl == 0) then 
			...
			setVehicleLightState ( BlinkerCar, 1, 1)
			setVehicleLightState ( BlinkerCar, 0, 1)	
		-- Rausgenommen, bis 1.0.4 drauen ist !!
	...
	for index, vehtable in pairs(blinkertable) do
		...
		if (getPedOccupiedVehicle ( vehinplayer ) == BlinkerCarLB) then 
			if (BlinkerStatusLB ~= 0) then
				...
				setPrivVehCorrectLightColor3 (BlinkerCarLB)
			else
	...
	for index, vehtable in pairs(blinkertable) do
		...
		if (getPedOccupiedVehicle ( vehinplayer ) == BlinkerCarRB) then 
			if (BlinkerStatusRB ~= 0) and (getVehicleLightState (BlinkerCarLB, 2) == 1) then
				...
				setVehicleLightState ( BlinkerCarLB, 3, 1 )
				setVehicleLightState ( BlinkerCarLB, 2, 1 )
				setPrivVehCorrectLightColor3 (BlinkerCarRB)
			else
	...
	for index, vehtable in pairs(blinkertable) do
		...
		if (getPedOccupiedVehicle ( vehinplayer ) == BlinkerCarWB) then 
			if (BlinkerStatusWB ~= 0) then
				...
				setPrivVehCorrectLightColor3 (BlinkerCarWB)
			else
	...
	if not(getVehicleType ( getPedOccupiedVehicle ( player ) ) == "Automobile") then
		if ( getPedOccupiedVehicleSeat ( player ) == 0 ) then
			...
			if ( getVehicleOverrideLights ( veh ) ~= 2 ) then	
				...
				setPrivVehCorrectLightColor3(veh)
		...
		if ( getPedOccupiedVehicleSeat ( player ) == 0 ) then
			local veh = getPedOccupiedVehicle ( player )
			if ( laGetElementData ( veh, "light") ~= true ) then	
				laSetElementData ( veh, "light", true)
			else	
				laSetElementData ( veh, "light", false)
			end
...
function setPrivVehCorrectLightColor3 ( veh )
