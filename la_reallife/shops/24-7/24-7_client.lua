itemShopItems = {
 ...
 handyItems = {
 ["50$ Guthaben"]=prepaid_low,
 ["100$ Guthaben"]=prepaid_middle,
 ["250$ Guthaben"]=prepaid_large
 }
...
itemShopIDs = {
 ...
 ["Rubbellos"]="los",
 ["Zigaretten"]="cig",
 ["50$ Guthaben"]="prepaid50",
 ["100$ Guthaben"]="prepaid100",
 ["250$ Guthaben"]="prepaid250"
 }
...
function create24_7Shop_func ()
	...
	if gWindow["24-7"] then
		...
		local row
		for key, index in pairs ( handyItems ) do
			row = guiGridListAddRow ( gGrid["24-7Items"] )
			guiGridListSetItemText ( gGrid["24-7Items"], row, gColumn["24-7Names"], key, false, false )
			guiGridListSetItemText ( gGrid["24-7Items"], row, gColumn["24-7Prices"], tostring ( index ).."$", false, false )
		end
		...
		gRadio["prepayed"] = guiCreateRadioButton(233,212,72,20,"PrePaid",false,gWindow["24-7"])
		...
		addEventHandler ( "onClientGUIClick", gButton["changeMobile24-7"],
			function ( btn, state )
				if state == "up" then
					...
					triggerServerEvent ( "changeTarif", lp, lp, val )
				end
			end,
		false )
