...
function packageLoad ( player )
	...
	for i = 1, 25 do
		...
		if paket == 1 then
			...
			laSetElementData ( player, "foundpackages", laGetElementData ( player, "foundpackages" ) + 1 )
		end
	end
end
...
function pickupPackage ( player )
	...
	if number then
		if tonumber ( laGetElementData ( player, "package"..number ) ) == 0 then
			local pname = getPlayerName ( player )
			laSetElementData ( player, "package"..number, 1 )
			outputChatBox ( "Du hast eins von 25 versteckten Päckchen gefunden und erhälst 10 Punkte sowie 100 $!", player, 0, 125, 0 )
			triggerClientEvent ( player, "showAchievmentBox", getRootElement(), " Päkchen\n gefunden!", 10, 10000 )
			laSetElementData ( player, "foundpackages", laGetElementData ( player, "foundpackages" ) + 1 )
			...
			MySQL_SetString("packages", "Paket"..number, laGetElementData ( player, "package"..number ), "Name LIKE '"..pname.."'")
...
function PackageAchievCheck ( player )
	...
	if laGetElementData ( player, "collectr_achiev" ) and laGetElementData ( player, "foundpackages" ) then
		if laGetElementData ( player, "collectr_achiev" ) ~= "done" and tonumber(laGetElementData ( player, "foundpackages" )) >= 25 then						-- Achiev: Collector
			laSetElementData ( player, "collectr_achiev", "done" )																								-- Achiev: Collector
			...
			MySQL_SetString("achievments", "DerSammler", laGetElementData ( player, "collectr_achiev" ), "Name LIKE '"..getPlayerName(player).."'")			-- Achiev: Collector
