...
function nameTagCheckPlayerSight ( player )
	...
	if isElement ( player ) then
		...
		nameTagImages[player]["army.png"] = false
		nameTagImages[player]["fbi.png"] = false
		nameTagImages[player]["la.png"] = false
		nameTagImages[player]["premium.png"] = false
		...
		if faction then
			...
			if armySkins[model] or fbiSkins[model] or copskins[model] then
				...
				elseif faction == 6 then
					nameTagImages[player]["fbi.png"] = true			
				elseif faction == 8 then
					nameTagImages[player]["army.png"] = true			
				end
			end
		end
		...
		if getElementData (player, "adminduty") == true then
			nameTagImages[player]["la.png"] = true
		end
		if getElementData (player, "premium") == true then
			if getElementData(player, "adminlvl") >= 2 then
				nameTagImages[player]["premium.png"] = false
			else
				nameTagImages[player]["premium.png"] = true
			end
		end
	else
			...
			if nameTagVisible[key] then
				...
				if x and y and z then
					...
					if sx and sy then
						...
						dxDrawText ( name, sx, sy, sx, sy, tocolor ( 0, 0, 0, 255 ), 2, "arial", "center", "center" )
						dxDrawText ( name, sx - 2, sy - 2, sx, sy, tocolor ( r, g, b, 255 ), 2, "arial", "center", "center" )
						dxDrawText ( social, sx, sy + 50, sx, sy, tocolor ( 0, 0, 0, 255 ), .8, "diploma", "center", "center" )
						dxDrawText ( social, sx - 2, sy - 1 + 50, sx, sy, tocolor ( 200, 200, 50, 255 ), .8, "diploma", "center", "center" )
						...
						for img, bool in pairs ( nameTagImages[key] ) do
							if bool then
								if images / 2 == math.floor ( images / 2 ) then
									dxDrawImage ( sx + 24 * ( drawn ) - images * 24 + 24, sy + 35, 24, 24, "images/nametag/"..img )
									drawn = drawn + 1
								else
									dxDrawImage ( sx + 24 * ( drawn ) - images * 24 / 2, sy + 35, 24, 24, "images/nametag/"..img )
									drawn = drawn + 1
								end
							end
						end
...
function calcRGBByHP ( player )
	...
	if hp <= 0 then
		...
		if armor > 0 then
			...
			return (100 + armor), (100 + armor), (100 + armor)
		else
		hp = math.abs ( hp - 0.01 )
