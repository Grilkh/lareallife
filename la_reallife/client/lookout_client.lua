...
for key, index in pairs ( lookouts["x"] ) do
	...
	local lookoutMarker = createMarker ( x, y, z - lookoutMarkerZDist, "cylinder", 1, 0, 255, 0, 150 )
	...
	addEventHandler ( "onClientMarkerHit", lookoutMarker,
		function ( hit )
			if hit == lp then
				...
				infobox ( "\n\nDrücke \"E\",\num das Fernglas zu benutzen.", 7500, 200, 200, 0 )
			end
		end
	)
end
...
function lookoutImage ( show )
	...
	if show then
		infobox ( "\n\nDrücke Enter,\num das Fernglas zu verlassen.", 7500, 200, 200, 200 )
