function laSetElementData ( element, field, value )
	return setElementData ( element, field, value )
end
function laGetElementData ( element, field )
...
--[[function guiDebugClick ()
	...
	if getElementType ( source ) == "gui-staticimage" and not internetImages[source] then
...
function anticheat ()
	...
	if getElementData ( lp, "loggedin" ) == 1 then
		...
		player_health = getElementHealth ( lp )
		if player_armor < getPedArmor ( lp ) then
			player = lp
			triggerServerEvent ( "armorcheater_ban", lp, player )
		end
		player_armor = getPedArmor ( lp )
...
function sec_gun_give ( weapon, ammo )
	...
	--[[for i = 1, 12 do
		_G["weapon"..i.."_ammo"] = getPedTotalAmmo ( lp, i )+9999
	end]]
end
