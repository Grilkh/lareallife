...
function loadFriendlist_func ()
	...
	local friendlist = xmlLoadFile ( ":testerver/friendlist/friendlist.xml" )
	if friendlist then
...
function showFriendlistSelf()
	...
	if gWindow["friendlistMenue"] then
		...
		gButton["friendAdd"] = guiCreateButton(0.728,0.1348,0.2308,0.0957,"Hinzufügen",true,gWindow["friendlistMenue"])
		...
		addEventHandler("onClientGUIClick", gGrid["friendlistNames"],
			function()
				...
				if row then
					...
					guiSetText ( gLabel["friendHandyNRValue"], "-" )
					guiSetText ( gLabel["FriendName"], "Bitte wählen!" )
				end
			end
		)
...
function deleteFriend ( friendname )
	...
	local friendlist = xmlLoadFile ( ":"..resourceName.."/friendlist/friendlist.xml" )
...
function fillFriendlist()
	...
	if gGrid["friendlistNames"] then
		...
		if not friendlist then
			local friendlist = xmlCreateFile ( ":"..resourceName.."/friendlist/friendlist.xml", "friendlist" )
