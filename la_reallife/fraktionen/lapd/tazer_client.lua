...
function onResourceStart()
    txd = engineLoadTXD ("mods/silenced.txd")
    engineImportTXD (txd, 347)
	ddf = engineLoadDFF("mods/silenced.dff",347)
...
function stopDamage ( attacker, weapon, bodypart )
	...
	cancelEvent()
		...
		if not (isTimer(tazeredTimer)) and (isInCar == false) then
			...
			nowtazered = true
			laSetElementData ( getLocalPlayer(), "tazered", true )
		...
		if not isTimer(dontChangeTimer) then
			if (getElementData(getLocalPlayer(), "Wpn2Norm") == 22) then
				if (getElementData(getLocalPlayer(), "Wpn2Ammo") >= 1) then
					...
					haveTaser = false
					preWeapon = 22
					outputChatBox("Deine Pistole (Schuss: "..getElementData(getLocalPlayer(), "Wpn2Ammo")..") wurde zu einem Taser (Schuss: "..getElementData(getLocalPlayer(), "Wpn2Ammo")..") geändert.")
				end
			end
			if (getElementData(getLocalPlayer(), "Wpn2Norm") == 24) then
				if (getElementData(getLocalPlayer(), "Wpn2Ammo") >= 1) then
					...
					haveTaser = false
					preWeapon = 24
					outputChatBox("Deine Deagle (Schuss: "..getElementData(getLocalPlayer(), "Wpn2Ammo")..") wurde zu einem Taser (Schuss: "..getElementData(getLocalPlayer(), "Wpn2Ammo")..") geändert.")
			...
			if (getElementData(getLocalPlayer(), "Wpn2Norm") == 23) then
				if (preWeapon == 22) then
					if (getElementData(getLocalPlayer(), "Wpn2Ammo") >= 1) then
						...
						dontChangeTimer = setTimer(dontChange, 5000, 1)
						haveTaser = false
						outputChatBox("Dein Taser (Schuss: "..getElementData(getLocalPlayer(), "Wpn2Ammo")..") wurde zu einer Pistole (Schuss: "..getElementData(getLocalPlayer(), "Wpn2Ammo")..") geändert.")
					end
				end
				if (preWeapon == 24) then
					if (getElementData(getLocalPlayer(), "Wpn2Ammo") >= 1) then
						...
						dontChangeTimer = setTimer(dontChange, 5000, 1)
						haveTaser = false
						outputChatBox("Dein Taser (Schuss: "..getElementData(getLocalPlayer(), "Wpn2Ammo")..") wurde zu einer Deagle (Schuss: "..getElementData(getLocalPlayer(), "Wpn2Ammo")..") geändert.")
					end
				end
			end
		else
