...
function startFirstRace_func ( player )
	...
	if player == client then
		if getPedOccupiedVehicle ( player ) == false then
			...
			laSetElementData ( player, "isInRace", true )
			laSetElementData ( player,"nodmzone", 1 )
...
function funexit_func ( player, veh )
	...
	if laGetElementData ( player, "isInRace" ) == true then
		if veh then
			destroyElement ( veh )
		end
...
function raceFinished_func ( player, ms, s, m )
	...
	if player == client then
		...
		if datatime <= 3000 then
			...
			laSetElementData ( player, "money", tonumber(laGetElementData ( player, "money" )) + pricemoney )
