		...
		if laGetElementData ( player, "carlicense" ) == 1 then
			if not raceStarted then
				if curSecRaceTotalPlayers <= 7 then
					...
					laSetElementData ( player, "secRaceID", index )
					...
					outputChatBox ( "Es werden maximal 8 Spieler unterstützt - bitte habe einen Moment Geduld!", player, 125, 0, 0 )
				end	
			else
				outputChatBox ( "Das Rennen ist bereits gestartet - bitte versuche es später noch einmal!", player, 125, 0, 0 )
			end
		else
...
function playerSecRaceExit ( player )
	...
	curSecRaceTotalPlayers = curSecRaceTotalPlayers - 1
	local id = laGetElementData ( player, "secRaceID" )
