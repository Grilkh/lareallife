...
function startRoulett_func ( string, count )
	...
	local player = client
	local betMoney = 0
	local playerChips = laGetElementData ( player, "casinoChips" )
	...
	if betMoney <= playerChips then
		if betMoney <= calcPlayerMaxGambleBet ( player, 0 ) then
			laSetElementData ( player, "casinoChips", playerChips - betMoney )
			...
			local diff = laGetElementData ( player, "casinoChips" ) - playerChips
			casinoAchievCheck ( player, diff )
			if diff > 0 then
				outputPayLog ( "[Roulett] "..getPlayerName ( player ).." hat "..diff.." Chips gewonnen", "casino" )
			elseif diff < 0 then
				outputPayLog ( "[Roulett] "..getPlayerName ( player ).." hat "..math.abs ( diff ).." Chips verloren", "casino" )
			end
			triggerClientEvent ( player, "showRoulettWinningNumber", player, roulettWinningNumber, laGetElementData ( player, "casinoChips" ) - playerChips )
		else
...
function roulettWin ( multiply, amount, player )
	...
	laSetElementData ( player, "casinoChips", laGetElementData ( player, "casinoChips" ) + multiply * amount )
end