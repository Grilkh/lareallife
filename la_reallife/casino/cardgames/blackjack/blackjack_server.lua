...
function startBlackJack_func ( amount )
	...
	if player == source then
		if not laGetElementData ( player, "blackJackStarted" ) then
			laSetElementData ( player, "blackJackStarted", true )
			...
			if amount <= chips then
				if amount <= calcPlayerMaxGambleBet ( player, 1 ) then
					if amount > 0 then
						laSetElementData ( player, "curBlackJackBet", amount )
						...
						if val == "21" then
							...
							laSetElementData ( player, "blackJackHand", true )
						else
							laSetElementData ( player, "blackJackHand", false )
							triggerClientEvent ( player, "startBlackJack", player, blackJackCards[player]["dealer"][1], blackJackCards[player]["player"][1], blackJackCards[player]["player"][2], val )
						end
					end
				else
...
function endBlackJackCurGame ( player, lost, blackJack )
	...
	if laGetElementData ( player, "blackJackStarted" ) then
		laSetElementData ( player, "blackJackStarted", false )
		local r, g, b, text = 0, 0, 0, ""
		local bet = laGetElementData ( player, "curBlackJackBet" )
		...
		if not lost then
			...
			if blackJack then
				...
				outputLog ( "[BLACKJACK]: "..getPlayerName ( player ).." hat "..(bet*2).." Chips gewonnen.", "pay" )
			else
				text = "Du gewinnst "..bet.." $"
				increasePlayerGambleSkillLevel ( player, bet )
				outputLog ( "[BLACKJACK]: "..getPlayerName ( player ).." hat "..bet.." Chips gewonnen.", "pay" )
			end
			if not blackJack then
				laSetElementData ( player, "casinoChips", laGetElementData ( player, "casinoChips" ) + bet * 2 )
			else
				laSetElementData ( player, "casinoChips", laGetElementData ( player, "casinoChips" ) + bet * 3 )
			end
		else
			casinoAchievCheck ( player, -1 * bet )
			text = "Du verlierst "..bet.." $"
			r = 150
			outputLog ( "[BLACKJACK]: "..getPlayerName ( player ).." hat "..bet.." Chips verloren.", "pay" )
		end
...
function blackJackDouble_func ()
	...
	if i == 6 then
		...
		local chips = laGetElementData ( player, "casinoChips" )
		if chips >= bet then
			laSetElementData ( player, "casinoChips", chips - bet )
			laSetElementData ( player, "curBlackJackBet", bet * 2 )
			local player = client
			local i = blackJackCards[player]["playerCardCount"] + 1
			blackJackCards[player]["playerCardCount"] = i
			blackJackCards[player]["player"][i] = getBlackJackCard ( player )
