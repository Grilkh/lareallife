...
function showHandy ()
	...
	if gWindow["handybg"] then
		...
		gWindow["handybg"] = guiCreateWindow(screenwidth/2-250/2,120,250,275,"Handy",false)
		
		guiSetAlpha(gWindow["handybg"],1)
		gLabel["eignummer"] = guiCreateLabel(0.096,0.1413,0.752,0.1739,"Eigene Handynummer: "..getElementData(getLocalPlayer(),"telenr"),true,gWindow["handybg"])
		...
		gButton["callfunc"] = guiCreateButton(0.544,0.2507,0.384,0.1467,"Anrufen",true,gWindow["handybg"])
		guiSetAlpha(gButton["callfunc"],1)
		gButton["smsfunc"] = guiCreateButton(0.096,0.2507,0.384,0.1467,"SMS",true,gWindow["handybg"])
		guiSetAlpha(gButton["smsfunc"],1)
		gButton["servicefunc"] = guiCreateButton(0.544,0.4409,0.384,0.1467,"Spezial",true,gWindow["handybg"])
		guiSetAlpha(gButton["servicefunc"],1)
		gButton["telefonbuch"] = guiCreateButton(0.096,0.4409,0.384,0.1467,"Telefonbuch",true,gWindow["handybg"])
		guiSetAlpha(gButton["telefonbuch"],1)
		
		gButton["handyonoff"] = guiCreateButton(0.096,0.6437,0.928,0.1067,"Ein/Ausschalten",true,gWindow["handybg"])
		guiSetAlpha(gButton["handyonoff"],1)
		
		gButton["handyopen"] = guiCreateButton(0.096,0.8037,0.928,0.1067,"Handy öffnen",true,gWindow["handybg"])
		guiSetAlpha(gButton["handyopen"],1)
		...
		addEventHandler("onClientGUIClick", gButton["handyopen"],
			function ()
				openthehandy_func ()
			end
		)
		addEventHandler("onClientGUIClick", gButton["servicefunc"],
			function ()
				outputChatBox ( "Polizei: 911, [LA]Leyynen: 147, Taxi: 400, Teufel: 666", 200, 200, 0 )
			end
		)
