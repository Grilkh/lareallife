...
function hideAllPlayerClickWindows ()
	...
	guiSetVisible ( gWindow["playerInteraktionTerror"], false )
...
function showJobMenues(button)
	if button == "left" then
		...
		elseif getElementData (lp, "job" ) == "hotdog" then
			...
			outputChatBox ( "Du hast einen ungültigen Beruf!", 125, 0, 0 )
		end
	end
end
...
function ShowInteraktionsguiGuiTerror_func ()

	if gWindow["playerInteraktionTerror"] then
		guiSetVisible ( gWindow["playerInteraktionTerror"], true )
	else
		gWindow["playerInteraktionTerror"] = guiCreateWindow(screenwidth/2-211/2,0,211,132,"Interaktion",false)
		guiSetAlpha(gWindow["playerInteraktionTerror"],1)
		
		gButton["playerInteraktionMelden"] = guiCreateButton(9,48,196,34,"Der Polizei melden",false,gWindow["playerInteraktionTerror"])
		guiSetAlpha(gButton["playerInteraktionMelden"],1)
		
		gButton["playerInteraktionWanted"] = guiCreateButton(9,87,60,34,"Wanted geben",false,gWindow["playerInteraktionTerror"])
		guiSetAlpha(gButton["playerInteraktionWanted"],1)
		
		gButton["playerInteraktionWeapons"] = guiCreateButton(75,87,65,34,"Entwaffnen",false,gWindow["playerInteraktionTerror"])
		guiSetAlpha(gButton["playerInteraktionWeapons"],1)
		
		gButton["playerInteraktionCloseTerror"] = guiCreateButton(145,87,60,34,"Schliessen",false,gWindow["playerInteraktionTerror"])
		guiSetAlpha(gButton["playerInteraktionCloseTerror"],1)
		
		gLabel["playerInteraktionInfo1Terror"] = guiCreateLabel(10,22,111,22,"Name des Spielers:",false,gWindow["playerInteraktionTerror"])
		guiSetAlpha(gLabel["playerInteraktionInfo1Terror"],1)
		guiLabelSetColor(gLabel["playerInteraktionInfo1Terror"],200,200,000)
		guiLabelSetVerticalAlign(gLabel["playerInteraktionInfo1Terror"],"top")
		guiLabelSetHorizontalAlign(gLabel["playerInteraktionInfo1Terror"],"left",false)
		guiSetFont(gLabel["playerInteraktionInfo1Terror"],"default-bold-small")
		
		gLabel["playerInteraktionClickedPlayerTerror"] = guiCreateLabel(121,23,82,20,"Unbekannt",false,gWindow["playerInteraktionTerror"])
		guiSetAlpha(gLabel["playerInteraktionClickedPlayerTerror"],1)
		guiLabelSetColor(gLabel["playerInteraktionClickedPlayerTerror"],125,200,200)
		guiLabelSetVerticalAlign(gLabel["playerInteraktionClickedPlayerTerror"],"top")
		guiLabelSetHorizontalAlign(gLabel["playerInteraktionClickedPlayerTerror"],"left",false)
		guiSetFont(gLabel["playerInteraktionClickedPlayerTerror"],"default-bold-small")
		
		
		addEventHandler ( "onClientGUIClick", gButton["playerInteraktionMelden"],
			function ( btn, state )
				if state == "up" then
					triggerServerEvent ( "melden", lp, lp, "melden", getElementData(lp,"curclicked") )
				end
			end,
		false )
		
		
		addEventHandler ( "onClientGUIClick", gButton["playerInteraktionWanted"],
			function ( btn, state )
				if state == "up" then
					triggerServerEvent ( "suspectterror", lp, lp, "suspectterror", getElementData(lp,"curclicked") )
				end
			end,
		false )
		
		addEventHandler ( "onClientGUIClick", gButton["playerInteraktionWeapons"],
			function ( btn, state )
				if state == "up" then
					triggerServerEvent ( "takeweapons", lp, lp, "takeweapons", getElementData(lp,"curclicked") )
				end
		...
		addEventHandler ( "onClientGUIClick", gButton["playerInteraktionCloseTerror"], hideAllPlayerClickWindows, false )
	end
end
addEvent ( "ShowInteraktionsguiGuiTerror", true )
addEventHandler ( "ShowInteraktionsguiGuiTerror", getRootElement(), ShowInteraktionsguiGuiTerror_func )