...
function ShowPoliceComputerGui_func()
	...
	for id, playeritem in ipairs(getElementsByType("player")) do 
		if getElementData ( playeritem, "loggedin" ) == 1 then
			if isOnStateDuty(playeritem) then
...
function SubmitWGebenBtn(button)
	if button == "left" then
		...
		triggerServerEvent ( "suspectGUI", getLocalPlayer(), getLocalPlayer(), guiGridListGetItemText ( gGrid["plist"], etplayer, column ), guiGetText ( gEdit["level"] ), fix, guiGetText ( gEdit["reason"] ) )
...
function SubmitWVorgegebenGebenBtn(button)
	if button == "left" then
		...
		local fix = "give"
		triggerServerEvent ( "suspectGUI", getLocalPlayer(), getLocalPlayer(), guiGridListGetItemText ( gGrid["plist"], etplayer, column ), wanteds, fix, grund )
		setTimer ( ShowPoliceComputerGui_func, 500, 1 )
	end
end
...
function _createPDComputerMenue ()
	...
	gGrid["players"] = guiGridListAddColumn(gGrid["plist"],"Bürger",0.5)
	...
	local row = guiGridListAddRow ( gGrid["WantedList"] )
	guiGridListSetItemText ( gGrid["WantedList"], row, gColumn["Verbrechen"], "Betreten des Bundeswehrgebiets", false, false )
	guiGridListSetItemText ( gGrid["WantedList"], row, gColumn["VerbrechenWanteds"], "4", false, false )	
	local row = guiGridListAddRow ( gGrid["WantedList"] )
	...
	guiGridListSetItemText ( gGrid["WantedList"], row, gColumn["VerbrechenWanteds"], "6", false, false )
	local row = guiGridListAddRow ( gGrid["WantedList"] )
	guiGridListSetItemText ( gGrid["WantedList"], row, gColumn["Verbrechen"], "Einbruch in die Bundeswehrbasis", false, false )
	guiGridListSetItemText ( gGrid["WantedList"], row, gColumn["VerbrechenWanteds"], "6", false, false )	
	local row = guiGridListAddRow ( gGrid["WantedList"] )
	guiGridListSetItemText ( gGrid["WantedList"], row, gColumn["Verbrechen"], "Einbruch in die Basis des FBIs", false, false )
	guiGridListSetItemText ( gGrid["WantedList"], row, gColumn["VerbrechenWanteds"], "6", false, false )	
	local row = guiGridListAddRow ( gGrid["WantedList"] )
	guiGridListSetItemText ( gGrid["WantedList"], row, gColumn["Verbrechen"], "Einbruch in die Garage des LAPDs", false, false )
	guiGridListSetItemText ( gGrid["WantedList"], row, gColumn["VerbrechenWanteds"], "6", false, false )	
	...
	local row = guiGridListAddRow ( gGrid["VerstaerkungList"] )
	guiGridListSetItemText ( gGrid["VerstaerkungList"], row, gColumn["VerstaerkungsGrund"], "Einbruch LAPD", false, false )
	local row = guiGridListAddRow ( gGrid["VerstaerkungList"] )
