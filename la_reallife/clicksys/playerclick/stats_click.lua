...
function createStatsWindow()
	...
	elseif buttonText == "Achiev-\nments" then
		...
		job = tonumber ( getElementData ( lp, "job" ) )
		job = jobNames[job]
		if not job then
			job = "Unbekannt"
		end
		...
		if getElementData ( lp, "gunlicense" ) == 1 then
			if getElementData ( lp, "gunlicenseB" ) == 1 then
				if getElementData ( lp, "gunlicenseC" ) == 1 then
					gl = "[C]"
				else
					gl = "[B]"
				end
		...
		local gwd = getElementData ( lp, "gwd" )
		if not gwd then
			gwd = "0%"
		else
			gwd = gwd.."%"
		end
		gWindow["stats"] = guiCreateWindow(screenwidth/2-477/2,120,477,370,"Persönliche Informationen",false)
		...
		gLabel["spielzeitWert"] = guiCreateLabel(80,66,109,16,playtime,false,gWindow["stats"])
		...
		gLabel["drogen"] = guiCreateLabel(20,213,43,16,"Weed:",false,gWindow["stats"])
		...
		gLabel["drogenWert"] = guiCreateLabel(69,213,109,18,tostring(getElementData(lp,"weed")),false,gWindow["stats"])
		...
		gLabel["paeckchen"] = guiCreateLabel(20,258,130,16,"Gefundene Päckchen:",false,gWindow["stats"])
		...
		gLabel["jobWert"] = guiCreateLabel(46,307,109,18,tostring( getElementData ( lp, "job" )),false,gWindow["stats"])
		...
		gLabel["fuehrerschein"] = guiCreateLabel(291,64,81,16,"Führerschein:",false,gWindow["stats"])
