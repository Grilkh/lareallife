function showFDepot_func ( fmoney, fmats, fweed, fkoks )
	...
	if gWindow["gangDepot"] then
		...
		addEventHandler("onClientGUIClick", gButton["DepotPut"],
			function ()
				...
				local weed = guiGetText ( gEdit["drugsDepot"] )
				local koks = guiGetText ( gEdit["koksDepot"] )
				triggerServerEvent ( "fDepotServer", getRootElement(), getLocalPlayer(), false, money, weed, mats, koks )
			end
		)
		addEventHandler("onClientGUIClick", gButton["DepotTake"],
			function ()
				...
				local weed = guiGetText ( gEdit["drugsDepot"] )
				local koks = guiGetText ( gEdit["koksDepot"] )
				triggerServerEvent ( "fDepotServer", getRootElement(), getLocalPlayer(), true, money, weed, mats, koks )
			end
		)
