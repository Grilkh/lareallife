...
createBlip ( -2502.216796875, 2361.5061035156, 5.2554173469543, 25, 1, 255, 0, 0, 255, 0, 200, getRootElement() )
...
pokerEntranceMarker = createMarker ( -2503.169, 2362.234, 3.959, "cylinder", 1.5, 125, 0, 0, 125 )
		...
		if getElementType ( hit ) == "player" then
			...
			if not getPedOccupiedVehicle ( hit ) and suc then
				...
				if pokerCurPlayers <= 5 then
					...
					outputChatBox ( "Bitte warte, alle Plätze werden zurzeit verwendet!", hit, 125, 0, 0 )
				end
			end
		end
	end
...
function nextPlayer ()
	...
	outputChatBox ( "Jetziger Einsatz: "..getHighestCall()..", your call: ".._G["pokerPlayer"..curPokerPlayer.."Bet"], _G["pokerPlayer"..curPokerPlayer], 150, 150, 25 )
	if isTimer ( timeoutTimer ) then
		killTimer ( timeoutTimer )
	end
