...
function showPokerHand_func ( card1, card2 )
	...
	if gWindow["pokerOwnCards"] then
		...
		gWindow["pokerOwnCards"] = guiCreateWindow(screenwidth-303,screenheight-20,303,265,"Eigene Hand",false)
...
function showPokerDisplay ()
	...
	if gWindow["pokerCommunity"] then
		...
		gWindow["pokerCommunity"] = guiCreateWindow(screenwidth/2-450/2,screenheight/2-256/2,450,259,"Tischkarten",false)
		...
		gButton["pokerCheck"] = guiCreateButton(0.2443+0.17*-1+0.0251*-1,0.695,0.17,0.1351,"Schieben",true,gWindow["pokerCommunity"])
		guiSetAlpha(gButton["pokerCheck"],0)
			...
			function ()
				if guiGetAlpha ( source ) == 1 and source == gButton["pokerCheck"] and block then
					if pokerCurBets == 0 or pokerCurBets == curOwnBet then
						...
						outputChatBox ( "Du kannst nicht schieben!", 125, 0, 0 )
					end
				end
			end
		)
		
		gButton["pokerCall"] = guiCreateButton(0.2443,0.695,0.17,0.1351,"Mitgehen",true,gWindow["pokerCommunity"])
		guiSetAlpha(gButton["pokerCall"],0)
		...
		gButton["pokerFold"] = guiCreateButton(0.2443+0.17+0.0251,0.695,0.17,0.1351,"Passen",true,gWindow["pokerCommunity"])
		guiSetAlpha(gButton["pokerFold"],0)
		...
		gButton["pokerRaise"] = guiCreateButton(0.2443+0.17*2+0.0251*2,0.695,0.1005,0.139,"Erhöhen",true,gWindow["pokerCommunity"])
		guiSetAlpha(gButton["pokerRaise"],0)
				...
				if guiGetAlpha ( source ) == 1 and source == gButton["pokerRaise"] and block then
					...
					if raiseAmount >= 10 then
						...
						if pokerCurOwnChips >= raiseAmount then
							...
							outputChatBox ( "Zuwenig Chips übrig!", 125, 0, 0 )
						end
					else
						outputChatBox ( "Mindestens 10!", 125, 0, 0 )
					end
				end
			end
		)
		
		gButton["pokerLeave"] = guiCreateButton(0.7717,0.8417,0.18491,0.1236,"Verlassen",true,gWindow["pokerCommunity"])
		guiSetAlpha(gButton["pokerLeave"],1)
