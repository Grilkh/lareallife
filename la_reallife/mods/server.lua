...
function togglePoliceLights (player)
	if isOnStateDuty (player) then
		bindKey ( player, "j", "down", toggleLights ) 
	end
end
addEventHandler ( "onVehicleEnter", getRootElement(), togglePoliceLights )
...
if laGetElementData ( player, "special" ) == true then
...
if isOnStateDuty (player) and curskin == 285 then
