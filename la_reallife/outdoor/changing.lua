		...
		if bridge1 then
				...
				for id, playeritem in ipairs(getElementsByType("player")) do
					if laGetElementData ( playeritem, "loggedin" ) == 1 then
						if tonumber(laGetElementData ( playeritem, "fraktion" )) == 5 then
							outputChatBox ("Hinweis: Brücke vom Highway nach Las Venturas wegen Wartungsarbeiten blockiert!",playeritem,255,100,0)
						end
					end
				end
		else
			...
			bridge1Object = createObject ( 4518, -2681.12890625, 1309.966796875, 56.183994293213, 0, 0, 270 )
				for id, playeritem in ipairs(getElementsByType("player")) do
					if laGetElementData ( playeritem, "loggedin" ) == 1 then
						if tonumber(laGetElementData ( playeritem, "fraktion" )) == 5 then
							outputChatBox ("Hinweis: Golden Gate Bridge vorübergehend wegen Bauarbeiten blockiert!!",playeritem,255,100,0)
						end
					end
				end
		end
		...
		if baustelle then
			killBaustelle()
				for id, playeritem in ipairs(getElementsByType("player")) do
					if laGetElementData ( playeritem, "loggedin" ) == 1 then
						if tonumber(laGetElementData ( playeritem, "fraktion" )) == 5 then
							outputChatBox ("Hinweis: Baustelle in San Fierro fertiggestellt!",playeritem,255,100,0)
						end
					end
				end
		else
			createBaustelle()
				for id, playeritem in ipairs(getElementsByType("player")) do
					if laGetElementData ( playeritem, "loggedin" ) == 1 then
						if tonumber(laGetElementData ( playeritem, "fraktion" )) == 5 then
							outputChatBox ("Hinweis: Es finden Bauarbeiten in San Fierro statt!",playeritem,255,100,0)
						end
					end
				end
		end
